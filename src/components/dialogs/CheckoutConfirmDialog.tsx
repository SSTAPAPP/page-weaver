    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-md p-0 overflow-hidden">
        <AlertDialogHeader className="px-5 pt-5 pb-3">
          <AlertDialogTitle className="text-lg">确认结账</AlertDialogTitle>
          <AlertDialogDescription asChild>
            <div className="space-y-0">
              {/* 顾客信息 */}
              <div className="flex items-center gap-3 rounded-md bg-muted/50 p-3 mt-2">
                ...
                <div className="flex-1">
                  ...
                  {!isWalkIn && memberBalance !== undefined && (
                    <p className="text-xs text-muted-foreground">余额 ¥{memberBalance.toFixed(2)}</p>
                  )}
                </div>
                {isWalkIn && (
                  <Badge variant="secondary" className="text-xs">非会员</Badge>
                )}
              </div>
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-sm font-medium text-muted-foreground">消费明细</p>
                  <span className="text-xs text-muted-foreground">{services.length} 项</span>
                </div>
                <div className="space-y-1">
                  {services.map((service, index) => (
                    ...
                    >
                      ...
                      <span className={`shrink-0 tabular-nums ${service.useCard ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
                        ¥{service.price}
                      </span>
                    </div>
                  ))}
        ...
        {/* 支付汇总区 - 灰底 */}
        <div className="bg-muted/30 border-t border-border px-5 py-3.5 space-y-2.5">
          {/* 支付方式明细 */}
        ...
        <div className="px-5 pb-5 pt-3 space-y-4">
          <div className="flex items-baseline justify-between">
            <span className="font-medium text-foreground">消费总计</span>
